...
<main class="home-wrap">
    ...
    <section>
        <div class="section-header">
            ...
            <a href="/customer/catalog.php" class="view-all">View All →</a>
        </div>
        <div class="book-grid">
            <?php
            // Get popular books (example: order by borrow_count or random)
            $stm = $_db->query("SELECT * FROM book ORDER BY book_id DESC LIMIT 4");
            $popular_books = $stm->fetchAll();
            ?>
            
            <?php if (empty($popular_books)): ?>
                <p class="empty-msg">No books available yet.</p>
            <?php else: ?>
